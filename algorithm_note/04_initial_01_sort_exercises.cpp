 ...
 *
 * 我的方法：
 *      将所有学生信息获取后进行一次排序，然后遍历一次学生信息数组，
 *      通过三个辅助数组算出学生在其考场的排名；
 *      每个辅助数组的大小是总考场数，分别保存遍历过程中当前遍历过的
 *      每个考场最后一名学生的成绩、考场排名和已排名学生的数量
 *
 * 书中的答案：
 *      在一个考场学生信息输入完成后对当前考场中所有的学生信息进行排
 *      序，并将考场排名保存在学生信息中；
 *      然后在所有学生信息录完之后在将所有的数据进行一次排序
 *
 * 方法对比：
 *      由于所有的排序使用的是C++的sort函数，这个函数是跟据不同情况
 *      选择不同的排序算法，因此无法判定排序的时间复杂度和产生的额外
 *      空间复杂度。
 *
 *      在时间复杂度上我的方法在排序操作上最少比书中的方法快一倍；
 *      因为我的方法中只进行了一次总体排序，和整体遍历，但书中的方法
 *      可以看作进行了两次整体排序和两次整体遍历（这里将所有的考场排
 *      序和遍历视为一次对整体数据的操作）
 *
 *      在空间复杂度上，在不考虑每次排序产生的额外空间复杂度的情况下，
 *      我的方法产生的额外空间复杂度可以记为：
 *      O(M)  M为考场的数量；
 *
 *      因此我的方法可以算作用时间换取空间（但是如果sort函数选择的排
 *      序策略产生了大于等于O(N)的额外空间复杂度，我的方法就有绝对的
 *      优势了，因为我方法中的O(M)比O(N)要小
 *      N * sizeof(stu.id) + (N-M) * sizeof(stu)）
 *
 * 运行对比：
 *      由于无法确定两种方法中sort函数选择的排序策略是否相同，所以两
 *      种方案的执行相率不能仅凭上面的时间复杂度来判定，所以我使用了
 *      8917个随机并乱序的数据（91个考场）对两种方分别进行了测试，采
 *      用CPU在执行过程中的累计的时钟滴答数进行计时（计时过程没有输
 *      出排序后的学生信息，以减小无用计时对数据结果的影响）。
 *      13次运行结果：
 *
 *      书中的方法：            我的方法：
 *          10430               8904
